 ...
 */
export function generateVerificationEmail(token: string, userEmail: string): string {
 ...
 */
export function generateResetPasswordEmail(token: string, userEmail: string): string {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000';
